...
- p1_a_detections.json   — per-frame detection records, consumed by every
...
- --hockey-model: HockeyAI (YOLOv8m fine-tuned on ice hockey, 7 classes).
  ...
  the p1_a_detections.json output schema stays uniform across backends:
...
def run(
    ...
    writes both an annotated MP4 and a per-frame ``p1_a_detections.json``.

    Args:
        video_path: Source MP4.
        output_dir: Where to write annotated_raw.mp4 + p1_a_detections.json. Created if missing.
    ...
    detections_path = output_dir / "p1_a_detections.json"
    ...
    for r in results:
        ...
        if r.boxes is not None and len(r.boxes) > 0:
            ...
            # never reach p1_a_detections.json.
