...
## Current status
- **Phase 1.a (Detect & track)** ✅ : dual-backend detection (YOLO11 COCO or
...
- **Phase 1.b (Teams)** ✅ : team clustering via pose-based torso crop
  ...
  based assignment (waits for Phase 3.a) or custom goalie sampling —
  deferred.
- **Phase 1.c (Numbers)** ✅ : per-track jersey-number OCR via **PARSeq
...
- **Phase 1.d (Entities)** ✅ : post-hoc **Re-ID clustering** that
  ...
  (from Phase 1.b), **strict temporal non-overlap**, and **OCR bonus**
  (from Phase 1.c). OCR conflicts are a hard block. Output:
...
- **Phase 1.e (Annotate)** ✅ : final MP4 with team-coloured boxes,
...
- **Phase 2.a (Follow-cam)** ✅ : virtual broadcast cam. Built but not
  heavily iterated on; runs in parallel to stages b–e (only depends
  on `detections.json`).
- **Phase 3.a (Rink calibration)** ❌ PARKED. The obvious shortcut
...
**Match vs training mode.** Phase 1.a enforces 1-puck-per-frame by default
...
### Jersey identification (Phase 1.c)
...
2. Build candidate merge graph: all pairs `(a, b)` with **same team_id
   from Phase 1.b** (with vote_confidence ≥ 0.67 on both sides), **zero
...
- Phase 1.b `is_goaltender` per track uses a **majority threshold**
  (>50 % of detections tagged `goaltender`), replacing the previous
  `any` rule that flipped a track on a single noisy frame.
- Phase 1.d entity-level `is_goaltender` is **weighted by frame
...
- Phase 1.c aggregation requires **≥2 agreeing votes** for the winning
...
slipped through anyway. The plan is to wait for **Phase 3.a rink
...
- **Puck**: rendered as a dark gray bbox (60,60,60) + short trace.
  No label.
- **Spectators**: not filtered (will be addressed by Phase 3.a rink
...
2. **Track fragmentation** is real but **partly absorbed** by Phase 1.d
   ...
   `player` or `goaltender` still pollute entities** — the Phase 1.b
   spectator filter attempted in run17 over-filtered real players and
   was reverted; clean fix waits for Phase 3.a rink calibration.
3. **Phase 3.a calibration** is blocked on annotation. HockeyRink (ice) does
...
5. **Refs leak into team clusters** because HockeyAI doesn't recognise
   ...
   Fixable by k=3 clustering in Phase 1.b or a dedicated ref detector.
...
- **run21 — Full pipeline avec PARSeq Hockey + RILH fine-tune sur
  ...
  * **Phase 1.c** : `--ocr-engine parseq --parseq-checkpoint
  ...
  * **Phase 1.d entités** : Video 04 → 24 entités (1 G, top-10 = 9 numéros
  ...
  * **Limitation restante** : noms de joueurs à 0 (PARSeq Hockey est
  ...
  → re-run OK.
- **run20 — Phase 1.c avec PARSeq Hockey baseline (sans fine-tune)
...
- **run19 — Collecte crops dorsal sur 4 vidéos pour annotation +
  ...
  (Video 04), run18 (Video 05). Phase 1.a nouveau sur clip60.mp4.
...
- **run17 — Villeneuve vs Vierzon (Video 05) — filtre spectator multi-signal.**
  Pipeline complet relancé. Phase 1.a inchangée vs run16 (HockeyAI +
  match-mode default, 1 puck/frame). Nouveauté Phase 1.b : `is_static` =
  ...
  * **Static tracks** : **261 / 435 (60 %)** flaggés spectator,
    excluded de Phase 1.c, 1.6, et annotate.
  * **Phase 1.c** : 1605 samples (vs 3696 run16, **-57 %
  ...
  * **Phase 1.d** : **20 entités** (vs 37 run16, **-46 %**).
  ...
  * **Hallucinations TrOCR persistent au niveau entity** : entity [0]
  ...
  calibration rink (Phase 3.a) qui répondra géométriquement « ce bbox
...
- **run16 — Villeneuve vs Vierzon (Video 05) — 5 fix consolidés.** Première
  ...
  vidéo finale. Phase 1.a rejouée (1h wall-clock), Phases 1.5/6/1.6/annotate
  neuves. Résultats :
  * **Palet** : 0 frame avec 2+ pucks ✓ (run15: 12.9% en avaient, max 5).
    260 → 185 puck tracks (filtrage des faux positifs).
  * **Goalies flag Phase 1.b** : 122 → **73 tracks** (-40%) via majority
    rule (>50% des détections tagées goaltender).
  * **Goalie entities Phase 1.d** : 17 → **5** (-71%) via frame-coverage
  ...
  * **Entités** : 36 → 37. Player groups (merge par #) : 16 → 11.
  ...
  median per-frame displacement < 5px (propagé Phase 1.b→6_identify→
...
- **run15 — Villeneuve vs Vierzon (Video 05) — debug mode + name OCR.**
  ...
  * Entités Phase 1.d : 36 (vs 37 run14). **17 taguées goalie** (9 du
...
- **run14 — Villeneuve vs Vierzon (Video 05, 60s, 1920×1080 @ 60fps).**
  ...
  * **Phase 1.a** ✅ HockeyAI + ByteTrack default. 3600 frames. 435 player
  ...
  * **Phase 1.b** ✅ HSV k=2 avec `--pose-model yolo26l-pose.pt` (premier
  ...
  * **Phase 1.c** ✅ TrOCR + yolo26l-pose. **59/435 (13,6 %)**,
  ...
  * **Phase 1.d** ✅ OSNet x0_25 + greedy merge. **435 tracks → 37
  ...
  * **Phase 1.e** ✅ entity-aware (37 entités couvrent 407 tracks).
...
- **run13 — Full pipeline + Phase 1.d + TrOCR on Video 04 (France vs
  Monde, 60s, 1920×1080 @ 30fps).** Phase 1.a (HockeyAI + ByteTrack default):
  167 player tracks (130 skaters + 37 goaltender fragments). Phase 1.b
  ...
  mixed-vote tracks). Phase 1.c iterated PARSeq→TrOCR:
  ...
  * **TrOCR** (same crops): 39/167 (**23.4%**), **10 player groups**.
  ...
  Phase 1.d (OSNet + greedy merge) on TrOCR output: **24 entities**
...
- **run12 — Full pipeline on Video 03 (Vierzon vs Pont de Metz, 30s,
  1920×1080 @ 60fps).** Phase 1.a (HockeyAI + ByteTrack default): 250 player
  ...
  tracks, puck in **28.6%** of frames. Phase 1.a ran ~22 min wall-clock (MPS
  + YOLOv8m @ 1280 on 60fps pushes compute). Phase 1.c: 26/250
  (**10.4%**), 4 player groups (#5, #7×2, #10). Phase 1.b iterated v1→v3:
...
- **run11 — Phase 1.a HockeyAI + BoT-SORT+ReID+GMC (clip60-2).** 349 player
  ...
  jerseys).
- **run10 — Phase 1.a HockeyAI + ByteTrack tuned (buffer=180, match=0.9).**
  312 player tracks, 196 puck tracks. ~28% fewer tracks than run04. Helps a
  bit; not enough.
- **run09 — Phase 1.c annotation v2.** Supervision-style boxes (green = team
  0, blue = team 1) + `#NN`/`#??` labels. Uses run04 tracks + run08
  identifications. `annotated_numbered.mp4` — user-validated look.
- **run08 — Phase 1.c OCR on run04 tracks.** 52/433 tracks identified (12%),
...
- **run07 — Phase 3.a HockeyRink transfer test on Video 02 (Vierzon vs
...
- **run06 — Phase 3.a HockeyRink transfer on Video 03 (Vierzon vs Pont de
  Metz, 30s).** 1/10 frames with detection, 19 keypoints but geometrically
  wrong. Same failure mode as run07.
- **run05 / run05b — Phase 3.a HockeyRink transfer on clip60-2.** Even with
...
- **run04 — Phase 1.a HockeyAI baseline (clip60-2, ByteTrack default).**
  This is **the canonical Phase 1.a output**. 17,860 player detections,
...
- **run02 — Phase 1.a COCO yolo11n baseline (clip60-2).** 63,224 person dets,
...
- **run01 — Phase 1.a first-ever run (clip60).** Crashed at ~35% of frames:
...
### Phase 1 — Court terme
- **Cache pose entre Phase 1.b et Phase 1.c** : aujourd'hui yolo26l-pose
  tourne 2× sur les mêmes frames. Écrire `pose_cache.json` en Phase 1.b
  et le lire en Phase 1.c sauve ~20-30 % wall-clock pipeline. — S
- **Phase 1.b + Phase 1.c en parallèle** (processus séparés). Gain
  ~10-20 % supplémentaire (overlap GPU/CPU). — S

### Phase 2 — Court-moyen terme
- **Fix team classification Phase 1.b** : sur run15, 6 tracks/435 mal
...
- **Phase 3.a** : calibration rink + map 2D. Bloqué sur 200–300 annotations
...
**Phase 3.a — Rink calibration & 2D map** (blocked on annotation)
...
- HockeyAI covers the player and puck cases reasonably (run04). Would
  ...
  Phase 1.c by producing longer tracks.
...
**Phase 1.c — Player identification** (partial)
- OCR paths implemented: PARSeq (default) + TrOCR (`--ocr-engine trocr`).
- Phase 1.d (entity clustering) is done and absorbs a lot of the
...
2. **Re-ID clustering post-hoc sous contrainte d'équipe** — *déjà implémenté
   en Phase 1.d*. Ciblait le vrai blocage actuel (fragmentation tracks
...
- Use HockeyAI (Phase 1.a output) to bootstrap annotations semi-automatically
...
- For Phase 3.a: ~200–300 frames with rink keypoints annotated (56-keypoint
...
- Camera too slow in Phase 2.a → raise `--alpha`. Camera jittery → lower it,
  or raise `--polish-window`.
- Still missing puck detections → `--imgsz 1280` or 1536 (slower but much
  better on small objects).
- Use `--debug-overlay` (Phase 2.a) to understand the focus trajectory.
