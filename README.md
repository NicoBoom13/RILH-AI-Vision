...
The identification pipeline is a strictly linear sequence `a → b → c → d → e`,
with `f` and `g` as optional separate outputs:

| Stage | Module | Status | Notes |
|-------|--------|--------|-------|
| **a — Detect & track** | `src/stage_a_detect.py` | ✅ | YOLO + ByteTrack. HockeyAI backend recommended. Match-mode default = max 1 puck/frame; pass `--training-mode` for drills. |
| **b — Teams** | `src/stage_b_teams.py` | ✅ | Pose-based torso crop + per-crop k=2 color vote. Goalie tagged via `class_name` majority threshold. |
| **c — Numbers** | `src/stage_c_numbers.py` | ✅ | PARSeq jersey OCR. Default = baudm pretrained; pass `--parseq-checkpoint models/parseq_hockey_rilh.pt` for our RILH-fine-tuned hockey model (test exact-match 97 %). |
| **d — Entities** | `src/stage_d_entities.py` | ✅ | OSNet medoid + greedy merge under team / non-overlap / OCR constraints. ~200 tracks → ~20–40 entities. Design doc: `docs/stage_d_entities_design.md`. |
| **e — Annotate** | `src/stage_e_annotate.py` | ✅ | Render the final MP4 with team-coloured boxes + `t{id} {G\|S} #NN` labels. |
| **f — Follow-cam** | `src/stage_f_followcam.py` | ✅ | Virtual broadcast cam. Works; not the current focus. |
| **g — Rink calibration** | `src/stage_g_rink.py` | ❌ parked | HockeyRink ice model doesn't transfer cleanly. Needs 200–300 annotated roller-rink frames to fine-tune. |

See `CLAUDE.md` for the full test log, design decisions, and open blockers.

## Scripts

- **`src/stage_a_detect.py`** — detection + tracking. Outputs `detections.json`.
- **`src/stage_b_teams.py`** — team classification from per-track jersey color. Outputs `teams.json` + `teams_preview.png`.
- **`src/stage_c_numbers.py`** — jersey-number OCR via PARSeq. Outputs `numbers.json`.
- **`src/stage_d_entities.py`** — entity Re-ID clustering. Outputs `entities.json`.
- **`src/stage_e_annotate.py`** — final video annotation. Reads everything above; outputs `annotated.mp4`.
- **`src/stage_f_followcam.py`** — broadcast follow-cam. Reads `detections.json`; outputs `followcam.mp4`.
- **`src/stage_g_rink.py`** — parked rink-calibration sanity check.
...
The end-to-end identification pipeline is `a → b → c → d → e`. Each stage
reads what the previous stages wrote into the same run folder.

### Stage a — Detect & track

```bash
# Recommended: HockeyAI for clean player + puck detection
python src/stage_a_detect.py path/to/match.mp4 --output runs/match01 --hockey-model

# Default: COCO YOLO11 (fast, but puck detection is very weak)
python src/stage_a_detect.py path/to/match.mp4 --output runs/match01
```
...
```bash
python src/stage_b_teams.py runs/match01/detections.json path/to/match.mp4 \
...
python src/stage_c_numbers.py runs/match01/detections.json path/to/match.mp4 \
  --pose-model yolo26l-pose.pt \
...
```bash
python src/stage_d_entities.py \
...
get a 10× merge bonus. See `docs/stage_d_entities_design.md`.
...
```bash
python src/stage_e_annotate.py \
  runs/match01/detections.json \
  runs/match01/numbers.json \
  path/to/match.mp4 \
...
### Stage f — Virtual follow-cam (optional, parallel)

```bash
python src/stage_f_followcam.py runs/match01/detections.json path/to/match.mp4 \
...
Even with HockeyAI, the puck is missed in ~50 % of frames. The follow-cam (`stage_f`) handles this with:
...
| **stage_d entity clustering** | **250 → 40 entities (run12), 167 → 24 (run13)** | Post-hoc OSNet embedding + team + non-overlap + OCR constraint. Works on top of any tracker. |

None of these hit the ideal ~12 entities on fragmented source video. stage_d takes you most of the way; the rest is capped by OCR recall on small/motion-blurred numbers and by ambiguous team colours — both source-quality issues.
...
- The `--debug-overlay` output (stage_f) is your best friend for understanding why the camera moves the way it does
